...
- **מודל הפצה דו-שכבתי (קריטי להבין):** (א) שכבת **Web** (app.html/today-v4/SW) חיה על Vercel → `git push` ל-master = **OTA אוטומטי לכל מי שכבר התקין**. (ב) שכבת **Native** (אייקונים/הרשאות/plugins/build.gradle) **קפואה ב-APK/AAB ולא מתעדכנת ב-OTA לעולם** — מגיעה למשתמשים רק ב-**build חדש שפורסם** לערוץ ההפצה (היום: Firebase App Distribution; בעתיד: Google Play — הכנה ב-`docs/play-store-readiness-he.md`). `adb install` = רק מכשיר הבדיקה שלי, לא משתמשים אחרים. **לכן:** אם תיקון כולל חלק native (למשל הרשאת exact-alarm), אותו חלק מגיע רק ב-build מפורסם, בעוד תיקוני JS באותה בעיה מגיעים מיד ב-OTA.
